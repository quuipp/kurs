...
public:
    ...
    //6 удалить иза массива amount элементов начиная с элемента index
    ...
    void copy_from_array_to_array(const char array_name_A, const char array_name_B, int index_of_start, int index_of_end)
    {
        if (array_name_A < 'A' || array_name_B > 'Z' || array_name_B < 'A' || array_name_A > 'Z')
        {
            throw std::invalid_argument("invalid array name");
    ...
    try
    {
        ...
        for (auto i = command_strings.cbegin(); i != command_strings.cend(); ++i)
        {
            ...
            else if (operation == "Stat")
            {
                ...
                std::cout << "Stats from array " << array << " are showed." << std::endl;
            }
            else if (operation == "Print")
            {
                ...
                if (!(holder == "all"))
                {
                    ...
                    std::cout << "Array's " << array << " elements from index " << start_index << " to " << end_index << " are printed." << std::endl;
                }
                else
                {
                    interpretator.print_all_from_array(array);
                    std::cout << "Array's " << array << " elements are printed." << std::endl;
                }
            }
            else
            {
